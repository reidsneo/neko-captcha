...
{
    /**
     * @var resource verification code image
    */
    protected $image;
    /**
     * @var string verification code text
    */
    protected $text;
    /**
     * @var string random character
    */
    ...
    */
    ...
    private $fonts = [];
    /**
     * @var int The number of verification code characters
    */
    private $number = 4;
    /**
     * @var int font size
    */
    private $fontSize = 24;
    /**
     * @var string verification code font
    */
    private $textFont;
    ...
    private $backColor;
    /**
     * @var bool Whether to add interference lines
    */
    private $isDrawLine = false;
    /**
     * @var bool whether to enable the curve
    */
    private $isDrawCurve = true;
    /**
     * @var bool Whether to enable background noise
    */
    ...
    public function create()
    {
        ...
        if (empty($this->fonts)) {
            throw new \Exception('The font does not exist');
        }
    ...
    */
    protected function drawSineLine() {
        ...
        $b = mt_rand(-$this->height/4, $this->height/4); // Y-axis direction offset
        $f = mt_rand(-$this->height/4, $this->height/4); // X-axis direction offset
        $T = mt_rand($this->height, $this->width*2); // period
        $w = (2* M_PI)/$T;

        $px1 = 0; // The starting position of the abscissa of the curve
        $px2 = mt_rand($this->width/2, $this->width * 0.8); // end position of curve abscissa
        ...
        for ($px=$px1; $px<=$px2; $px = $px + 1) {
            if ($w!=0) {
                ...
                while ($i > 0) {
                    imagesetpixel($this->image, $px + $i , $py + $i, $color); // here (while) cycle draws pixels than imagettftext and imagestring with font size once (without this while loop) much better performance
                    $i--;
                }
            }
        }
    ...
    */
    ...
    /**
    * draw noise
    * Write letters or numbers of different colors on the picture
    */
